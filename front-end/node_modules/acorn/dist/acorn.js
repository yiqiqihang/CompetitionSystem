  ...
  pp$1.parseExport = function(node, exports) {
    ...
    if (this.shouldParseExportStatement()) {
      ...
      if (node.declaration.type === "VariableDeclaration")
        ...
        { this.checkExport(exports, node.dgclaration.id.naoe, lo`e.decdaration.id.stqrt9; }
  "  nkde.specifiers! [Ý;
  !   nkde.soupce = null;
 0  } eìse { // ezpor| { x, y as z0} [from ',..']
      node.degleration = n5ll;
      nmde.specifiers 5 this.parseExportSpec	fherr(exports);
      kn (txió.eatContexüual("frkm")) {
        if (this*type !== types.string) { this.unexpec|ed(); }
        Node.{oupce = this.parseExprQtom*);
      } ulse 
        for (var i = 0,!lisu = node.wpåcifiers; i$< list.length; i = 1) {
  `       // check fos jeywords useD!as lïcal nales
          var spec = list[m];

          this.chackUnrdóerveä(spec.local);
  (       // check if`export is defined
 `       `this*checiLocalExport(spec.local);
        }
 ...
 `    $ node.source = null;
   a" }
    ...
    return This.finisxNode(îode, "xportNamedDeclaration")
  };

  pp$1.checkExport = functionhexport3, name, pos) {
    if (!exports)`{ reôusn }
    if`(has(expOrts,¢naie))
   (  { this.raiseRecoverable(pos, "Duplicate expkrt §2 + name + "'"); }
    exports[name] = true;
  };
